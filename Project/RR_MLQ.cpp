...
public:
    ...
    int cpu_burst1, original_cpu_burst1;
    int io_burst;
    int cpu_burst2;
    string burst_type;
    // derived variables
    ...
    PCB(int _PID, string _name, bool _cpu_bound, int _priority, int _arrival_time, int _burst_time, int _io_burst, int _burst_time2)
    {
        ...
        original_cpu_burst1 = _burst_time;
        io_burst = _io_burst;
...
public:
    ...
    void print_schedule()
    {
        ...
        {
            cout << setw(10) << pcb.PID << setw(10) << pcb.name << setw(10) << pcb.priority << setw(10) << pcb.arrival_time / 1000 << setw(10) << pcb.original_cpu_burst1 / 1000 << setw(10) << pcb.io_burst / 1000 << setw(10) << pcb.cpu_burst2 / 1000 << setw(10) << pcb.start_time / 1000 << setw(10) << pcb.response_time / 1000 << setw(10) << pcb.completion_time / 1000 << setw(10) << pcb.turn_around_time / 1000 << setw(10) << pcb.waiting_time / 1000 << endl;
        }
    }
    void save_schedule(bool is_append)
    {
        ...
        for (PCB pcb : terminated)
        {
            fout << pcb.PID << "," << pcb.name << "," << pcb.priority << "," << pcb.arrival_time / 1000 << "," << pcb.original_cpu_burst1 / 1000 << "," << pcb.io_burst / 1000 << "," << pcb.cpu_burst2 / 1000 << "," << pcb.start_time / 1000 << "," << pcb.response_time / 1000 << "," << pcb.completion_time / 1000 << "," << pcb.turn_around_time / 1000 << "," << pcb.waiting_time / 1000 << endl;
        }
        fout.close();
    }
    int execute(vector<string> process_input, int cur_time, bool is_append)
    {
        current_time = cur_time;
        populateReadyQueue(process_input);
        performTasks();
        print_schedule();
        save_schedule(is_append);
        return current_time;
    }
};